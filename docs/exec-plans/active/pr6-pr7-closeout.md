...
1. [ ] Review PR #6 invariants, fix only blocking defects, run relevant tests and full CI, then merge #6. Local review and verification complete; remote CI and merge pending.
2. [ ] Refresh main and converge PR #7, preserving both sets of behavior and existing uncommitted user work.
...
- Remote CI after the new commit: pending.

## Next action

Commit and push the focused PR #6 fixes, wait for GitHub CI, then merge if the PR remains clean. Preserve current publishing adapters and all user changes in the separate PR #7 worktree.
