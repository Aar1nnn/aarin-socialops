...
export default async function SettingsPage() {
  ...
  const [client, accounts, integrations, usage, channels] = await Promise.all([
    ...
    db.socialAccount.findMany({ where: { clientId: context.clientId }, orderBy: { platform: "asc" } }),
  ...
  return (
    <OperatorShell context={context}>
      ...
      <div className="grid">
        ...
        <section className="card span-5"><h2>使用量</h2><div className="metric">{(usage._sum.inputUnits || 0) + (usage._sum.outputUnits || 0)}</div><p className="muted">输入与输出单位合计；上限 {client.usageMonthlyLimit}。模拟生成记录为 0 且带 simulated 标志。</p><div className="warning">切换到正式模式不会绕过适配器验证。本阶段没有真实发布适配器，任务会进入待配置。</div></section>
