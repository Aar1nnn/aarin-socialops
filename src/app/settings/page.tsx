  ...
  return (
    <OperatorShell context={context}>
      ...
      <div className="grid">
        ...
        <section className="card span-12"><h2>通知通道</h2><div className="grid"><div className="span-7"><table><thead><tr><th>类型</th><th>名称</th><th>状态</th><th>验证时间</th></tr></thead><tbody>{channels.map((channel) => <tr key={channel.id}><td>{channel.type}</td><td>{channel.displayName}</td><td>{channel.status}</td><td>{channel.verifiedAt?.toLocaleString("zh-CN") || "未验证"}</td></tr>)}</tbody></table></div><form className="stack span-5" action="/api/notifications/channels" method="post"><label>类型<select name="type"><option value="WEBHOOK">Webhook</option><option value="EMAIL">Email HTTP endpoint</option></select></label><label>通道名称<input name="displayName" required placeholder="例如：运营告警 Webhook" /></label><label>服务端端点变量引用<input name="credentialRef" required pattern="env:[A-Z][A-Z0-9_]+" placeholder="env:NOTIFICATION_WEBHOOK_URL" /></label><label><input type="checkbox" name="confirmVerified" value="true" /> 已人工验证端点与收件范围</label><button disabled={context.role !== "OWNER"}>保存通知通道</button></form></div><p className="muted">外部端点只从服务端环境变量读取。失败会记录在 NotificationDelivery，不会回滚站内通知或主业务。</p></section>
      </div>
    </OperatorShell>
  );
}
