...
export function ContentActions(props: ActionProps) {
  ...
  return <section className="content-action-stack" aria-label="内容操作">
    ...
    {!props.hasConfirmedFacts ? <Notice title="没有已确认产品事实" tone="warning">{props.manualContent ? "这是人工创建的内容；人工审核者必须核对正文事实，AI 改写仍需要可验证的产品上下文。" : "先在产品页确认事实，再生成或审核内容。AI 不会把待确认事实作为可靠资料。"}</Notice> : null}
